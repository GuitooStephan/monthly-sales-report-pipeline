...
# Importing tasks
from tasks.extract_websites_names_from_config_files_paths import get_website_names
...
@dag(
    ...
    dag_id='generate_products_monthly_sales_with_std_config'
)
def generate_products_monthly_sales_with_std_config():
    """
    This dag generates a monthly sales report for each product and loads it to a database or a csv file
    The products data originates from our ecommerce parterns database
    ...
    2. Standardize the data of each website - columns and file names using a config file
    3. Join the data of each website
    4. Clean the data
    5. Generate the monthly sales report
    6. Load the monthly sales report to a database or a csv file - ./include/data/output/*.csv
    """
...
dag = generate_products_monthly_sales_with_std_config()
